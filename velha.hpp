...
int verifica_vencedor(const std::vector<std::vector<int>> &jogo) {
    int soma_jog = 0;
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            soma_jog += jogo[i][j];
    ...
    if (soma_jog == 18 || (soma_jog == 9 && abs(qntd1 - qntd2) > 1 )) {
        // Caso teste 1 todas as casa cotém o número 1
        // ou 2 e suas somas respectivamente são 9 e 18
        return -2;
    }
    ...
    if (testZeroeUm && testUmeDois && testDifZeroUmDois) {
        // Diagonal principal
        return jogo[0][0];
    }
    ...
    if (testDoiseUm && testUmeZero && testDifDoisUmZero) {
        // Diagonal secundaria
        return jogo[0][2];
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        ((jogo[i][0] + jogo[i][1] + jogo[i][2]) >= 3)) {  // vencedor da linha
            return jogo[i][0];
        }
        if ((jogo[0][i] == jogo[1][i]) && (jogo[1][i] == jogo[2][i]) &&
        ((jogo[0][i] + jogo[1][i] + jogo[2][i]) >= 3)) {
            // vencedor da coluna
            return jogo[0][i];
        }
    }
